...
async function placeOrder(userInfo) {
  ...
  const validate = validateOrderUser(userInfo)
  if(!validate.isValid){
...
async function updatePaymentStatus(orderId, isPaid) {
  ...
  try {
    const update =await updateOrderStatus(orderId, isPaid)
...
async function removeOrder(orderId) {
  ...
  // 回傳格式：{ success: true, data: ... } / { success: false, error: ... }
  try {
    const remove = await deleteOrder(orderId)
...
function displayOrders(orders) {
  ...
  order.foreach((item,index) => {
    const format = formatOrder(item)
    console.log(`${index+1}`)
    console.log("----------------------------------------")
    console.log(`訂單編號：${item.id}`)
    ...
    console.log(`訂單金額：NT$ ${item.totalFormatted}`)
    console.log(`付款狀態：${item.paidText}`)
    console.log(`建立時間：${item.createdAt} (${item.dayAgo} 天前)`)
    console.log("----------------------------------------")
    console.log("商品明細：")
