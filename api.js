...
async function fetchCart() {
  ...
  return {carts: res.data.carts, total: res.data.total , finalTotal: res.data.finalTotal}
}
...
async function updateCartItem(cartId, quantity) {
  // 請實作此函式
  const res = await axios.patch(`${BASE_URL}/api/livejs/v1/customer/${API_PATH}/carts`,{data:{cartId:cartId, quantity:quantity}})
  return res.data
}
...
async function createOrder(userInfo) {
  // 請實作此函式
  const res = await axios.post(`${BASE_URL}/api/livejs/v1/customer/${API_PATH}/carts`,{data:{user:userInfo}})
  return res.data
}
